...
export const PlanButton = ({ 
  ...
  const handleSubscribe = async () => {
    ...
    setIsLoading(true);
    try {
      await createCheckoutSession(planType as "iniciante" | "investidor" | "iniciante_anual" | "investidor_anual");
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`inline-flex items-center justify-center w-full font-bold py-3 px-6 rounded-lg transition-colors disabled:opacity-50 cursor-pointer ${buttonStyle}`}
    >
      {isLoading ? (
        <>
        ...
        <>
          {children}
          <div className="ml-2 w-6 h-6 bg-current/20 rounded flex items-center justify-center">
